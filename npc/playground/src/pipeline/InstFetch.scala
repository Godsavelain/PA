...
class InstFetch extends Module{
  ...
  io.out.bits.inst := Mux(imem_stall , 0.U ,Mux(use_reg_info ,inst_out ,resp.bits.rdata))
  io.out.bits.inst_valid := Mux(imem_stall ,false.B ,Mux(use_reg_info ,valid_out, resp.bits.rvalid))
