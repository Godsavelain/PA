...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("?????????????????????????     11011 11", jal    , J, s->dnpc = s->pc + src1; R(dest) = s->snpc; if((dest == 1) ) ftrace_call(s->pc,s->dnpc););
  INSTPAT("????????????  ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src2 + src1) & (word_t)(-1); R(dest) = s->snpc; if((dest == 0) && (src2 == 0) && (BITS(s->isa.inst.val, 19, 15) == 1)) ftrace_ret(s->pc,s->dnpc););
