...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("????????????  ????? 010 ????? 00100 11", slti   , I, R(dest) = (src1 < (long long int)src2) ? 1 : 0);
  INSTPAT("????????????  ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (src1 < src2) ? 1 : 0);
  INSTPAT("????????????  ????? 000 ????? 00110 11", addiw  , I, R(dest) = SEXT(BITS((src1 + src2) ,31 ,0 ), 32)  );
  INSTPAT("????????????  ????? 000 ????? 01100 11", sub    , R, R(dest) = (src1 - src2) ) ; 
  INSTPAT("????????????  ????? 000 ????? 01110 11", addw   , R, R(dest) = SEXT(BITS((src2 - src1) ,31 ,0 ), 32)  );
  INSTPAT("?????????????????????????     11011 11", jal    , J, s->dnpc = s->pc + src1; R(dest) = s->snpc);
  INSTPAT("????????????  ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src2 + src1) & (word_t)(-1); R(dest) = s->snpc);
